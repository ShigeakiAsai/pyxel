...
class TestFont:
    ...
    def test_ttf(self, assets_dir):
        font = pyxel.Font(str(Path(assets_dir) / "PixelMplus10-Regular.ttf"), 10)
        assert font.text_width("A") > 0

    def test_ttf_different_sizes(self, assets_dir):
        font_small = pyxel.Font(str(Path(assets_dir) / "PixelMplus10-Regular.ttf"), 8)
        font_large = pyxel.Font(str(Path(assets_dir) / "PixelMplus10-Regular.ttf"), 20)
