...
- Audit a file's code-side aspects even when its prose content is handed off for separate work; the handoff covers content, not the file.
...
- Do not add documentation comments (Rust `///`, Python docstrings) anywhere except `python/pyxel/__init__.pyi`; API documentation is maintained separately.
  - e.g., `python/pyxel/__init__.pyi` docstrings are regenerated by `scripts/generate_pyi_docstrings`; do not hand-edit
...
The audit runs as ordered phases that gate one another; the meta-rules apply throughout.
...
2. Run the cross-file consistency check; verify every file pair or group sharing a concern as an explicit matrix row with evidence, and count a pair left out as a skipped check.
...
5. Gate completion with `superpowers:verification-before-completion`; author the matrix and plan with `superpowers:writing-plans`.
