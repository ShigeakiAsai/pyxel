...
- Do not add documentation comments (Rust `///`, Python docstrings). API documentation is maintained separately.
  - Exception: `python/pyxel/__init__.pyi` — its docstrings are regenerated by `scripts/generate_pyi_docstrings` (do not hand-edit), and its default values reflect what the Rust implementation applies when arguments are omitted, not the PyO3 binding layer's defaults.
